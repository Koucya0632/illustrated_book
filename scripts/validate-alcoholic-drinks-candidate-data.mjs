...
if (!draftFile && (!entriesFile || !spansFile)) {
  console.error(
    "Usage: node scripts/validate-alcoholic-drinks-candidate-data.mjs (--draft <file> | --entries <file> --spans <file>) [--expected-count <n>]",
  );
  process.exit(2);
}
...
for (const entry of entries ?? []) {
  ...
  if (entry.category !== "alcoholic-drinks") issues.push(`${entry.id}: category must be alcoholic-drinks`);
