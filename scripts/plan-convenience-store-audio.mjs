...
const output = path.resolve(
  ...
  outputArg?.slice("--output=".length) ||
    "output/convenience-store-audio-plan/manifest.json",
);
const batches = ["a", "b"];
const entries = batches.flatMap((batch) => {
  const file = path.join(
    root,
    `data/convenience-store-series-2026-09-batch-${batch}.json`,
  );
...
const issues = [];
if (entries.length !== 34) issues.push(`expected 34 entries, found ${entries.length}`);
...
if (jobs.length !== 306) issues.push(`expected 306 jobs, found ${jobs.length}`);
