...
const sourceRoot = path.join(
  root,
  "output/alcoholic-drinks-publish-prep/source-images",
);
const outputRoot = path.join(
  root,
  "output/alcoholic-drinks-publish-prep/images",
);
...
const publicUrl = (storagePath) =>
  `https://img.nexflow.team/word-images/${storagePath}`;

fs.mkdirSync(outputRoot, { recursive: true });
const images = [];
for (const entry of entries) {
  if (entry.id === "sake") {
    const existingFile = path.join(sourceRoot, "sake-existing.webp");
    const bytes = fs.readFileSync(existingFile);
    const metadata = await sharp(bytes).metadata();
    entry.imageUrl = "https://img.nexflow.team/word-images/sake.webp";
    images.push({
      id: entry.id,
      mode: "reuse-existing",
  ...
  const sourceName = sourceVersion.get(entry.id) ?? `${entry.id}-v1.png`;
  const sourceFile = path.join(sourceRoot, sourceName);
  if (!fs.existsSync(sourceFile)) {
    throw new Error(`${entry.id}: missing selected source image ${sourceName}`);
  }
...
const categorySource = path.join(
  sourceRoot,
  "category-alcoholic-drinks-v1.png",
);
...
const categoryStoragePath = `category-alcoholic-drinks-ai-${categorySha256.slice(0, 12)}.webp`;
...
const manifestFile = path.join(
  root,
  "output/alcoholic-drinks-publish-prep/image-manifest.json",
);
  ...
  JSON.stringify(
    {
      manifestFile,
      ...manifest.summary,
      categoryUrl: manifest.category.url,
    },
    null,
    2,
  ),
