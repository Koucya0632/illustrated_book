...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  let body: { publicItemId?: unknown };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "invalid body" }, { status: 400 });
  }
  const publicItemId = typeof body.publicItemId === "string" ? body.publicItemId : "";
  if (invalidId(publicItemId)) return NextResponse.json({ error: "invalid item" }, { status: 400 });
  ...
  // Not added means the guard rejected it (not owner / not approved / wrong
