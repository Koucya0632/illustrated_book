// Guarded one-time publisher for the reviewed 2026-09 clothing series.
//
// Default mode is a SELECT-only dry run:
//   npm run clothing:publish
//
...
//   npm run clothing:publish -- --apply \
...
//   npm run clothing:publish -- --rollback <apply-manifest.json> \
//     --confirm-host <database-fingerprint> \
//     --confirm-storage <storage-fingerprint>
...
const DEFAULT_PLAN_FILE = path.resolve(
  ROOT,
  "output/clothing-publish-prep/publish-plan.json",
);
...
function verifyPreparedPlan(plan: any, planBytes: Buffer) {
  ...
  if (
    ...
    throw new Error("invalid or incomplete clothing publish plan");
  }
...
async function assertStoredContentMatchesPlan(
  ...
  `;
  assertRowSet("clothing category", categoryRows, [expectedCategory(plan)]);
  const categoryTranslations = manifest.categoryExistedBefore
    ...
    : await tx<Json[]>`
        ...
        FOR UPDATE
      `;
  assertRowSet("clothing category translation", categoryTranslations, [{
    category_id: plan.category.id,
    language: "ja",
    name: plan.category.nameJa,
    description: plan.category.descriptionJa,
...
async function stageStorage(manifestFile: string, manifest: ApplyManifest) {
  ...
  for (const [index, object] of manifest.storageObjects.entries()) {
    ...
    if ((index + 1) % 25 === 0 || index + 1 === manifest.storageObjects.length) {
      console.log(`[clothing:publish] Storage ${index + 1}/${manifest.storageObjects.length}`);
    }
  }
...
async function publishDraft(tx: Tx, plan: any, manifest: ApplyManifest) {
  ...
  if (states.length !== plan.summary.words || states.some(({ status }) => status !== "draft")) {
    throw new Error(`publish refused: all ${plan.summary.words} clothing rows must still be draft`);
  }
  ...
  const [row] = await sql<Record<string, number>[]>`
    SELECT
      ...
      (SELECT count(*)::int FROM user_settings
        WHERE study_category = 'clothing'
           OR 'clothing' = ANY(regexp_split_to_array(study_categories, '\\s*,\\s*'))
      ) AS user_settings
  `;
  return row;
}
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      if (states.length !== ids.length) {
        throw new Error("rollback refused: only part of the clothing series exists");
      }
      const phase = states.every(({ status }) => status === "published")
        ...
        : states.every(({ status }) => status === "draft")
          ? "draft"
          : null;
      if (!phase) throw new Error("rollback refused: clothing row states are mixed");
      ...
      if (outsideReuse[0].count > 0) {
        throw new Error("rollback refused: a clothing sentence is now reused outside the series");
      }
      ...
      if (sharedDefinitions.length > 0) {
        throw new Error("rollback refused: a clothing sentence is reused by an outside definition");
      }
      ...
      if (inboundRelations.length > 0) {
        throw new Error("rollback refused: outside words now link to the clothing series");
      }
      ...
      if (outsideSpanLinks.length > 0) {
        throw new Error("rollback refused: outside sentence spans now link to the clothing series");
      }
      await tx`
    ...
    writeJsonAtomic(manifestFile, manifest);
    console.log(
      `[clothing:publish] rolled back ${ids.length} words; ` +
...
async function main() {
  ...
  if (
    actionManifest &&
    (actionManifest.schemaVersion !== 1 || actionManifest.series !== "clothing")
  ) {
    throw new Error("invalid clothing recovery manifest");
  }
  ...
  try {
    ...
    if (!APPLY) {
      console.log(
        `[clothing:publish] dry run only; no Storage or database changes. ` +
    ...
    if (preflight.alreadyPublished && !RESUME_FILE) {
      throw new Error("all clothing rows are already published; use the audit workflow instead");
    }
    if (preflight.resumableDraft) {
      throw new Error(
        "committed clothing drafts are from an obsolete publisher flow; rollback them before continuing",
      );
    }
    ...
    try {
      ...
      if (preflight.alreadyPublished) {
        ...
        writeJsonAtomic(manifestFile, manifest);
        console.log(
          `[clothing:publish] recovered completed publication; manifest: ${relative(manifestFile)}`,
        );
        return;
      }
      ...
      writeJsonAtomic(manifestFile, manifest);
      console.log(`[clothing:publish] published ${plan.summary.words} words; manifest: ${relative(manifestFile)}`);
    } catch (error) {
...
main().catch((error) => {
  console.error("[clothing:publish] failed:", error);
  process.exitCode = 1;
});
