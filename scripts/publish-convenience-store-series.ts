// Guarded one-time publisher for the reviewed convenience-store series.
//
...
// words and updates the existing convenienceStore row in one serializable transaction,
// preserving convenienceStore's word ID, example IDs, and card IDs.
...
const DEFAULT_PLAN_FILE = path.resolve(ROOT, "output/convenience-store-publish-prep/publish-plan.json");
...
interface ApplyManifest {
  ...
  convenienceStoreBeforeDigest: string;
  convenienceStoreAfterDigest?: string;
...
async function snapshotConvenienceStore(sql: Sql | Tx) {
...
async function inspectDatabase(sql: Sql, plan: any) {
  ...
  const convenienceStoreRow = candidateRows.find(({ id }) => id === EXISTING_ID);
  ...
  `;
  const convenienceStore = await snapshotConvenienceStore(sql);
  const convenienceStoreBeforeDigest = digest(convenienceStore);
  const existingSpanKeys = new Set(
    (convenienceStore.spans as Json[]).map(
  ...
  const alreadyApplied =
    ...
    candidateRows.every(({ status, category: categoryId }) =>
      status === "published" && categoryId === "convenience-store",
    );
  return {
    missingTables,
    candidateRows,
    ...
    writeReady:
      ...
      ((fresh &&
        convenienceStoreRow?.status === "published" &&
        convenienceStoreRow?.category === "street" &&
        convenienceStoreBeforeDigest === plan.databasePreflight.convenienceStoreBeforeDigest &&
...
function verifyPreparedPlan(plan: any, planBytes: Buffer) {
  if (
    ...
    throw new Error("invalid or incomplete convenience-store publish plan");
  }
...
function resolveConvenienceStoreExampleIds(plan: any) {
  const oldExamples = plan.databasePreflight.convenienceStoreBefore.examples as Array<{ id: string; sort_order: number }>;
  if (oldExamples.length !== 2) throw new Error("convenienceStore must have exactly two stable example IDs");
  return Object.fromEntries(oldExamples.map((example) => [`${EXISTING_ID}:${example.sort_order}`, String(example.id)]));
}
...
async function stageStorage(manifestFile: string, manifest: ApplyManifest) {
  ...
  for (const [index, object] of manifest.storageObjects.entries()) {
    ...
    if ((index + 1) % 25 === 0 || index + 1 === manifest.storageObjects.length) {
      console.log(`[convenience-store:publish] Storage ${index + 1}/${manifest.storageObjects.length}`);
    }
  }
...
async function updateConvenienceStoreCore(tx: Tx, item: any, plan: any, exampleIds: Record<string, string>) {
  const old = plan.databasePreflight.convenienceStoreBefore;
  ...
  for (const example of item.examples) {
    ...
    const updated = await tx<{ id: string }[]>`
      ...
      RETURNING id::text
    `;
    if (updated.length !== 1) throw new Error(`${example.ownerKey}: stable convenienceStore example ID disappeared`);
    await tx`
      INSERT INTO word_example_translations (example_id, language, translation)
      VALUES (${exampleId}, 'ja', ${example.ja}), (${exampleId}, 'zh', ${example.zh})
    `;
  ...
  await tx`
    INSERT INTO word_categories (word_id, category_id, is_primary)
    VALUES (${EXISTING_ID}, 'convenience-store', TRUE)
  `;
}
  ...
  if (item.operation === "update-existing") {
    for (const card of [englishCard, japaneseCard]) {
      const updated = await tx<{ id: string }[]>`
        ...
        RETURNING id::text
      `;
      if (updated.length !== 1) throw new Error(`convenienceStore/${card.deck_key}: expected one stable card`);
    }
  } else {
    for (const card of [englishCard, japaneseCard]) {
      await tx`
...
async function verifyAppliedContent(tx: Tx, plan: any, exampleIds: Record<string, string>) {
  ...
  assertRowSet(
    "category links",
    snapshot.categories,
    ids.map((wordId: string) => ({ word_id: wordId, category_id: "convenience-store", is_primary: true })),
  );
  return snapshot;
}
...
async function applyRelease(tx: Tx, plan: any, manifest: ApplyManifest) {
  ...
  `);
  const currentConvenienceStore = await snapshotConvenienceStore(tx);
  if (digest(currentConvenienceStore) !== manifest.convenienceStoreBeforeDigest) {
    throw new Error("apply refused: the convenienceStore catalogue snapshot changed after review");
  }
  const newIds = plan.releaseShape.insertWordIds as string[];
  ...
  const convenienceStore = plan.entries.find((item: any) => item.word.id === EXISTING_ID);
  await updateConvenienceStoreCore(tx, convenienceStore, plan, manifest.resolvedExampleIds);
...
async function restoreConvenienceStore(tx: Tx, plan: any) {
  const old = plan.databasePreflight.convenienceStoreBefore;
  ...
  for (const row of old.cards) {
    ...
    `;
    if (updated.length !== 1) throw new Error(`rollback refused: convenienceStore card ${row.id} changed identity`);
  }
}
...
async function rollback(sql: Sql, manifestFile: string, manifest: ApplyManifest, planFile: string, plan: any) {
  ...
  if (!manifest.convenienceStoreAfterDigest || !manifest.insertedSeriesAfterDigest) {
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      `);
      const currentConvenienceStore = await snapshotConvenienceStore(tx);
      if (digest(currentConvenienceStore) !== manifest.convenienceStoreAfterDigest) {
        throw new Error("rollback refused: convenienceStore changed after the release");
      }
      ...
      await restoreConvenienceStore(tx, plan);
      await tx`DELETE FROM categories WHERE id = ${plan.category.id}`;
      const restoredConvenienceStore = await snapshotConvenienceStore(tx);
      if (digest(restoredConvenienceStore) !== manifest.convenienceStoreBeforeDigest) {
        throw new Error("rollback verification failed: convenienceStore was not restored exactly");
      }
    });
    ...
    writeJsonAtomic(manifestFile, manifest);
    console.log("[convenience-store:publish] database rolled back; content-addressed Storage retained");
  } catch (error) {
...
async function main() {
  ...
  if (actionManifest && (actionManifest.schemaVersion !== 1 || actionManifest.series !== "convenience-store")) {
    throw new Error("invalid convenience-store recovery manifest");
  }
  ...
  try {
    ...
    console.log(JSON.stringify({
      ...
      preservedSpanRows: preflight.spanCollisions.length,
      unexpectedSpanCollisions: preflight.unexpectedSpanCollisions.length,
      categoryExists: Boolean(preflight.category),
      convenienceStoreBeforeDigest: preflight.convenienceStoreBeforeDigest,
      convenienceStoreSnapshotSections:
        preflight.convenienceStoreBeforeDigest === plan.databasePreflight.convenienceStoreBeforeDigest
          ? "match"
          : Object.fromEntries(
              Object.keys(preflight.convenienceStore).map((key) => [key, {
                live: digest((preflight.convenienceStore as any)[key]),
                planned: digest(plan.databasePreflight.convenienceStoreBefore[key]),
              }]),
            ),
    ...
    if (!APPLY) {
      console.log(
        `[convenience-store:publish] dry run only; no Storage or database changes. ` +
    ...
    try {
      if (Object.keys(manifest.resolvedExampleIds).length === 0) {
        manifest.resolvedExampleIds = {
          ...resolveConvenienceStoreExampleIds(plan),
      ...
      const afterConvenienceStore = await snapshotConvenienceStore(sql);
      const newEntries = plan.entries.filter((item: any) => item.operation === "insert");
      ...
      manifest.convenienceStoreAfterDigest = digest(afterConvenienceStore);
      manifest.insertedSeriesAfterDigest = digest(afterNew);
      manifest.status = "applied";
      manifest.appliedAt ??= new Date().toISOString();
      manifest.updatedAt = new Date().toISOString();
      writeJsonAtomic(manifestFile, manifest);
      console.log(`[convenience-store:publish] published 34-word series; manifest: ${relative(manifestFile)}`);
    } catch (error) {
...
main().catch((error) => {
  console.error("[convenience-store:publish] failed:", error);
  process.exitCode = 1;
});
