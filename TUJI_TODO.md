...
### 遊戲化
- **XP / 等級系統**（原型的 `LV 8 · 探索家`、升級進度條）。目前資料模型沒有 XP/等級；側欄與各 hero 改用「圖鑑完成度」真實數字代替。→ 需要 `user_xp` / 等級規則。
- **勳章 / 成就 `勳章`**（24 個成就）。`/progress`、`/me` 目前是寫死的視覺佔位（標「即將推出」）。→ 需要成就定義 + 判定引擎 + `user_achievements`。
