...
const planPath = resolve(repoRoot, "output/drugstore-audio-plan/manifest.json");
const audioRoot = resolve(repoRoot, "output/drugstore-audio-candidates");
...
if (plan.series !== "drugstore" || manifest.series !== "drugstore") {
  issues.push("plan and generated manifest must both identify drugstore");
}
...
const report = {
  ...
  checks: ["plan membership", "path containment", "byte count", "SHA-256", "MP3 header", "ffprobe metadata", "full ffmpeg decode"],
  issues,
