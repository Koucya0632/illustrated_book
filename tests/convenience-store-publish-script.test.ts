...
const publisher = readFileSync(
  new URL("../scripts/publish-convenience-store-series.ts", import.meta.url),
  "utf8",
);
const preparer = readFileSync(
  new URL("../scripts/prepare-convenience-store-publish-plan.ts", import.meta.url),
  "utf8",
);
...
test("the convenience-store publisher defaults to a SELECT-only dry run", () => {
  ...
  assert.match(packageJson.scripts["convenience-store:publish"], /publish-convenience-store-series\.ts/);
});
...
test("convenienceStore is compare-and-swap updated without changing word, example, or card IDs", () => {
  assert.match(publisher, /the convenienceStore catalogue snapshot changed after review/);
  assert.match(publisher, /`\$\{EXISTING_ID\}:\$\{example\.sort_order\}`/);
...
test("the release publishes 33 drafts only after all 34 records and media rows are assembled", () => {
  ...
  const convenienceStore = publisher.indexOf("updateConvenienceStoreCore(tx, convenienceStore, plan, manifest.resolvedExampleIds)");
  ...
  assert.ok(insert > 0 && convenienceStore > insert && media > convenienceStore && publish > media && verify > publish);
...
test("rollback is CAS guarded and preserves content-addressed Storage", () => {
  assert.match(publisher, /rollback refused: convenienceStore changed after the release/);
  ...
  assert.match(publisher, /restoreConvenienceStore\(tx, plan\)/);
...
test("the plan records the mixed release shape and exact rollback source", () => {
  ...
  assert.match(preparer, /convenienceStoreBeforeDigest/);
  assert.match(preparer, /Preserve existing convenienceStore card IDs/);
});
