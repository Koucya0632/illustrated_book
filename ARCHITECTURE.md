...
```
┌────────────────────────────────┐   ┌─────────────────────────────┐
│  Browser (web app)             │   │  iOS native app (tuji-ios)  │
│  ├── Tuji Shell (Shell.tsx)    │   │  ├── WordsStore / Categories│
│  │   - sidebar / tab bar / 吉祥物│   │  │   Store（打公開 JSON API）│
│  │   - Words/Categories/User   │   │  ├── Supabase Swift SDK     │
│  │   - SettingsProvider        │   │  │   (email/password/OAuth) │
│  │   - I18nProvider            │   │  ├── Authorization: Bearer  │
│  │     (zh-Hant / zh-Hans)     │   │  │   <access_token>         │
│  │   - learningDirection 切換   │   │  ├── APNs push token 註冊    │
│  │     (zh-en / zh-ja)         │   │  └── StudyReport 回報        │
│  │   - localStorage cache      │   └──────────────┬──────────────┘
│  │   - 發音：預生成 MP3 + Web Speech│                  │
...
│    ├─ Authorization: Bearer    → service-role 驗 JWT → x-user-id   │
│    └─ everything else          → refresh Supabase session cookie  │
│                                                                   │
│  Server Components ────► lib/current-user.ts ─┐ (header-first:     │
│  Public JSON (iOS):                            │  x-user-id)       │
│    /api/words  /api/words/:id  /api/categories │                  │
│  API routes (Node):                            │                  │
│    /api/events  /api/search                    │                  │
│    /api/users/{me,profile,settings,favorites,  ├─► @supabase/ssr  │
│                learned,sync,progress,mastery,  │   (cookie) +      │
│                top-words,push-token,           │   service-role    │
│                delete-account}                 │   (Bearer)        │
│    /api/study/{queue,answer,stats,reports}     │                  │
│    /api/admin/{words,words/:id,words/:id/enrich,                  │
│                upload,fetch-image,reports/:id} │                  │
...
│  cards (image-en / image-ja), events  (RLS: public read)         │
│                                                                  │
│  Storage:  word-images bucket (public read)                      │
│            word-audio  bucket (public read; Chirp 3 HD MP3)      │
│            ── 自家託管，runtime 不依賴任何外部圖床 / 音床          │
└──────────────────────────────────────────────────────────────────┘

外部：
  accounts.google.com  ─ OAuth (managed by Supabase)
  Google Cloud TTS     ─ scripts/generate-audio.ts only:
                         Chirp 3 HD 預生成發音 → word-audio bucket
...
| User Auth | `@supabase/ssr`（web cookie）+ `@supabase/supabase-js` + service-role（mobile Bearer） | email/password、Google OAuth、session cookie 全內建；iOS 走 `Authorization: Bearer` |
| Mobile | iOS 原生（`../tuji-ios`，Swift）打 Vercel 的公開 JSON API + Bearer 私有 API | 共用同一套後端與 DB；無獨立服務 |
| Admin Auth | DIY HMAC cookie + `ADMIN_SECRET` | 單一密碼，跟用戶系統解耦 |
| Speech | 預生成 **Chirp 3 HD** MP3（Google Cloud TTS）+ Web Speech API fallback | US/UK/JA 口音；同一發音人；無 runtime AI |
| 圖片 / 音檔 | **Supabase Storage**（`word-images` + `word-audio` public buckets） | 自家託管、無外部 runtime 依賴 |
| AI（admin / scripts） | `ai` SDK v6 經 Vercel AI Gateway，預設 `anthropic/claude-sonnet-4-6` | enrich / translate；前台 runtime 不會打 AI |
| i18n | `lib/i18n.ts`（UI）+ `lib/opencc.ts`（zh-Hans 即時轉換）+ DB overlays | **介面**兩語：zh-Hant（source）/ zh-Hans；**學習內容**英 / 日由 `learningDirection` 決定（與 UI 語言解耦） |
...
│   ├── admin/reports/        # 學習回報審核（ReportActions）
│   └── admin/stats/
├── login/                    # admin 單一密碼登入
└── api/
    ├── auth/login | logout                  # admin
    ├── auth/callback                        # Supabase OAuth code exchange
    ├── words | words/[id]                   # 公開單字 JSON（iOS WordsStore；CDN 快取）
    ├── categories                           # 公開分類 JSON（iOS CategoriesStore）
    ├── users/{me, profile, settings,
    │          favorites, learned, sync,
    │          progress, mastery, top-words,
    │          push-token, delete-account}   # user accounts + per-user data（含 iOS）
    ├── admin/words(/:id)                    # CRUD
    ├── admin/words/[id]/enrich              # 單字 AI 補齊（etymology / forms / chinese_def）
    ├── admin/reports/[id]                   # 回報狀態 PATCH
    ├── admin/upload | fetch-image           # 圖檔
    ├── events                               # 公開埋點（node）
    ├── search                               # 全站搜尋（pg_trgm）
    ├── study/{queue, answer, stats, reports}# SRS + 學習回報提交
    ├── test_smoke/whoami                    # 暫時：驗 Bearer header 路徑
...
├── PronunciationButton.tsx   # 優先播預生成 MP3（audioUrls[locale]）→ Web Speech fallback
├── SettingsProvider.tsx      # draft + save、reset on login change（含 learningDirection）
├── StudyReportModal.tsx      # 學習回報浮層 → POST /api/study/reports
├── UserProvider.tsx
├── WordsProvider.tsx         # server 拉好的字庫放 context
...
├── data.ts                   # 公開 read path：words/CardWord（+ getLearningWord，依 learningDirection）
├── cache-headers.ts          # 公開 GET 的 ETag/304 + publicJson（搭 next.config.js Cache-Control）
├── words.ts                  # 108 字靜態 seed（legacyToV2 normalizer）
...
├── settings.ts               # UserSettings（含 learningDirection）+ targetLanguageFor / studyDeckFor / normalize
...
├── supabase/{client,server,middleware}.ts   # SSR 三 clients；middleware 含 Bearer 驗證 + x-user-id
├── analytics.ts              # client sendBeacon
├── speech.ts                 # Web Speech API + 口音切換
├── storage.ts                # localStorage + dual-write to server
├── i18n.ts                   # UI 字典 zh-Hant / zh-Hans（ja 已退場為 UI 語言）
...
└── image-urls.json           # seed 圖片 URL 對照

scripts/
├── migrate.ts                # DDL + seed + 卡片 generator；vercel-build 跑（非 production build 自動 skip）
├── enrich.ts                 # 批次：撈 etymology IS NULL → AI gateway → 寫回
├── translate.ts              # 批次翻譯：缺語系的 word_definitions / category_translations（--audit-only 只稽核）
├── backfill-ja-google.ts     # 日文釋義回填（Google 翻譯）
├── backfill-ja-readings.ts   # 日文 reading（假名）回填
├── generate-audio.ts         # Chirp 3 HD 預生成發音 → word-audio bucket + word_media（idempotent）
├── normalize-word-images.ts  # word-images 統一邊距正規化
├── translate-list.ts         # 列出待翻譯項目（無 npm script）
├── translate-apply.ts        # 套用人工翻譯回 DB（無 npm script）
...
| `/admin/*` | `middleware.ts` 攔截 | Admin CRUD + 統計 + AI 補齊 + 學習回報審核（`/admin/reports`） |
...
### 公開（CDN 快取 — iOS native + web 共用）

| Method | Path | Runtime | 用途 |
|---|---|---|---|
| GET | `/api/words?lang=&learning=` | node | 全部已發布單字 lite JSON（`CardWord`）；`revalidate 300` + ETag/304；`Cache-Control` 在 `next.config.js` 設 |
| GET | `/api/words/[id]?lang=&learning=` | node | 單字完整 JSON（study peek modal + iOS）；依 `learning` 給 en/ja 目標詞 |
| GET | `/api/categories?lang=` | node | 全部分類 + 多語名稱（iOS CategoriesStore） |
| POST | `/api/events` | node | view / favorite / pronounce / quiz_attempt；IP SHA-256 前 8 bytes |
| GET | `/api/search?q=...` | node | pg_trgm 模糊查詢 words.word + word_definitions |

### 使用者帳號 + per-user 資料（cookie 或 Bearer）

| Method | Path | 公開？ | 用途 |
|---|---|---|---|
| GET | `/api/users/me` | cookie / Bearer | 取個人 bundle |
| PATCH | `/api/users/profile` | 需登入 | 暱稱 / 頭像 |
| GET / PUT | `/api/users/settings` | 需登入 | `user_settings`：dailyGoal / accent / showZh / studyCategories / studyDecks / **learningDirection** / uiLang / fontSize |
| GET / POST | `/api/users/favorites` | 需登入 | 我的收藏 |
| POST | `/api/users/learned` | 需登入 | 標記已學（依 learningDirection 寫 target_language 分區） |
| GET | `/api/users/mastery` | 需登入 | 全字熟練度 map + `nextReviewAt`（讀時套衰減；iOS 圖鑑/詳情徽章） |
| GET | `/api/users/top-words?type=best\|weak&limit=` | 需登入 | Me 頁 Top / 弱字（join words） |
| POST / DELETE | `/api/users/push-token` | 需登入 | 註冊 / 移除 APNs token（per device_id） |
...
> Supabase Auth 走 `@supabase/ssr`：`signUp`、`signInWithPassword`、`signInWithOAuth({provider:"google"})` 由 client SDK 直打，不再有 `/api/users/register|login|logout` 路由。iOS 用 Supabase Swift SDK 拿 access token，往後每個請求帶 `Authorization: Bearer`。

### SRS

| Method | Path | 用途 |
|---|---|---|
| GET | `/api/study/queue?mode=new\|review&limit=...[&cats=...][&decks=...]` | 取到期 + 新卡；附 MCQ `choices`（4 選 1）與 `spellingChoices`（Step 3 候選詞）；按 `user_settings.studyCategories / studyDecks` 過濾；deck 依 `learningDirection`（image-en / image-ja）；mastery ASC 排弱字優先 |
| POST | `/api/study/answer` | `{cardId, rating}` → 排下次複習 + 更新 mastery + 寫 study_logs |
| GET | `/api/study/stats` | 30d 趨勢 / today 已學 / 完成率 |
| POST | `/api/study/reports` | 提交學習回報（issue_type / description / 題目 snapshot）；`request_id` 冪等；驗 card_id ↔ word_id |
...
| PATCH | `/api/admin/reports/[id]` | middleware | 學習回報狀態（pending/reviewing/resolved/rejected/duplicate）+ internal_note |
...
> 之後再加入：**雙語學習**（`word_terms` 目標詞 + `cards.deck_key='image-ja'`，並把 `user_learned` /
> `user_words` / `study_logs` 以 `target_language` 分區，讓同一人學英 / 日各有獨立進度）、**學習回報**
> （`study_reports`）、**推播 token**（`user_push_tokens`）、以及 **per-locale 發音**（`word_media.locale`）。
...
word_terms                    -- 目標語言「要學的詞」（英 / 日各一列）
  word_id FK CASCADE, language TEXT      -- 'en' | 'ja'
  term TEXT,                              -- 'en'=words.word；'ja'=日文詞
  reading TEXT NULL,                      -- 假名讀音
  pronunciation TEXT NULL, audio_url TEXT NULL, updated_at
  PK (word_id, language)
  INDEX (language, word_id)
  -- migrate 由 words.word（en）+ ja 釋義（ja）回填；image-ja 卡片由此產
...
word_media                    -- 圖 / 音 / AI 記憶圖（多筆同 kind 用 is_primary 挑預設）
  ...
  locale TEXT NULL,             -- audio 專用：'en-US' | 'en-GB' | 'ja-JP'
  is_primary BOOL, sort_order INT, metadata JSONB
  UNIQUE INDEX (word_id, kind) WHERE is_primary
  UNIQUE INDEX (word_id, kind, locale) WHERE kind='audio'  -- 每 locale 一條音檔
  -- 由 scripts/generate-audio.ts UPSERT；en-US 同步鏡像回 words.audio_url
...
user_settings     -- 一個 user 一列；首次保存才寫進來
  ...
  learning_direction TEXT DEFAULT 'zh-en'  -- 'zh-en' | 'zh-ja'（學英 / 學日；與 ui_lang 解耦）
                       CHECK IN ('zh-en','zh-ja')
  ui_lang   TEXT DEFAULT 'zh-Hant'    -- 'zh-Hant' | 'zh-Hans'（ja 已退場為 UI 語言）
  font_size TEXT DEFAULT 'md'         -- 'sm' | 'md' | 'lg'
  updated_at

user_favorites    user_id FK CASCADE, word_id FK CASCADE, created_at  PK (user_id, word_id)
user_learned      user_id FK CASCADE, word_id FK CASCADE, target_language TEXT DEFAULT 'en',
                  learned_at   PK (user_id, word_id, target_language)  -- 英 / 日各自記已學
...
cards
  ...
  deck_key  TEXT             -- 'image-en'（看圖選英文）| 'image-ja'（看圖選日文）
...
user_cards
  ...
  -- 方向分區靠 cards.deck_key（image-en vs image-ja），不需另存 target_language

user_words                    -- 單字熟練度（跨卡型；EMA + lazy decay）
  user_id, word_id, mastery NUMERIC(5,2), target_language TEXT DEFAULT 'en',
  last_reviewed_at, review_count, updated_at
  PK (user_id, word_id, target_language)            -- 英 / 日各自一份熟練度
  INDEX (user_id, target_language, mastery)
...
study_logs                    -- append-only：每次答題的完整快照
  ...
  target_language TEXT DEFAULT 'en'   -- 'en' | 'ja'（英 / 日學習各自的快照流）
  client_session_id TEXT, metadata JSONB
  INDEX (user_id, created_at DESC), (user_id, target_language, created_at DESC)

study_reports                 -- 使用者回報的內容 / 產品問題（web + iOS）
  id BIGSERIAL PK, request_id UUID UNIQUE,  -- request_id 冪等去重
  user_id FK CASCADE, word_id FK SET NULL, card_id FK SET NULL,
  issue_type CHECK IN ('image','content','audio','answer','ui','other')
  description TEXT (1..1000), mode CHECK IN ('new','review'), phase,
  selected_answer, platform CHECK IN ('web','ios'), app_version, ui_lang,
  snapshot JSONB,             -- 出題當下的題面快照（字被改後仍可追溯）
  status DEFAULT 'pending' CHECK IN ('pending','reviewing','resolved','rejected','duplicate')
  internal_note, duplicate_of FK SET NULL, created_at, updated_at
  INDEX (status, created_at DESC), (word_id, issue_type, created_at DESC),
        (user_id, created_at DESC)

user_push_tokens              -- APNs / FCM 推播 token（每用戶每裝置一列）
  user_id FK CASCADE, device_id TEXT, token TEXT,
  platform DEFAULT 'ios', updated_at, PK (user_id, device_id)
  INDEX (user_id)
  -- 目前只做 token 註冊；伺服器端推播 fanout 尚未實作
```
...
- 刪 `auth.users` 一個 row → `profiles` + 所有 `user_*`（含 `user_push_tokens`）+ `study_logs` + `study_reports` cascade 清空
- 刪 `words` 一個 row → `cards`、`word_definitions`、`word_terms`、`word_examples` (→ translations)、`word_tags`、`word_relations`(source 端)、`word_media`、`word_categories`、`word_localized_texts`、`user_favorites`、`user_learned`、`user_words`、`user_cards`（透過 cards）、`study_logs` 全部 cascade；`study_reports.word_id / card_id` 為 `SET NULL`（回報保留）
...
|---|---|---|
| `words`、`categories`、`category_translations`、`tags`、`word_tags`、`word_definitions`、`word_terms`、`word_examples`、`word_example_translations`、`word_relations`、`word_media`、`word_categories`、`word_localized_texts`、`cards` | 公開 `USING(true)` | service-tier only |
| `events` | 公開 `USING(true)` | anon `WITH CHECK(true)` |
| `profiles` | 公開 `USING(true)` | self UPDATE `auth.uid()=id` |
| `user_settings / user_favorites / user_learned / user_cards / user_words / user_push_tokens` | self ALL `auth.uid()=user_id` | 同 |
| `study_logs / study_reports` | self SELECT `auth.uid()=user_id` | self INSERT `auth.uid()=user_id` |
...
Server: lib/current-user.ts（header-first）
  → 讀 middleware 蓋的 x-user-id；缺才 fallback supabase.auth.getUser()
  → 返回 user.id → 查 profiles + per-user 表
```

`middleware.ts` 每次請求 `updateSupabaseSession()`：驗 cookie（順手刷 token）後把 `user.id` 寫進 request header `x-user-id`，下游 route handler / server component 直接讀，**省掉每路由重複一次 `getUser()`**（單次 ~170–645ms）。`getCurrentUserId()` 是 header-first，`getCurrentUserIdFast` 現為其 alias（deprecated）。

### Mobile（iOS — `Authorization: Bearer`）

iOS 不帶 cookie，改帶 Supabase access token：

```
iOS（Supabase Swift SDK）
  → signIn / OAuth 拿 access_token（自動 refresh）
  → 每個請求帶  Authorization: Bearer <access_token>

middleware.ts updateSupabaseSession():
  ↓ 先 req.headers.delete('x-user-id')   ← 防偽造身分
  ↓ 有 Bearer → service-role admin.auth.getUser(token) 驗 JWT
  ↓ 通過 → req.headers.set('x-user-id', user.id)；不碰 cookie
  ↓ （Bearer 無效則 fall through 走 cookie 路徑）
```

公開 JSON（`/api/words`、`/api/categories` …）不需登入；per-user（`/api/users/*`、`/api/study/*`）靠這條 Bearer 路徑拿 `x-user-id`。`/api/test_smoke/whoami` 是驗這條路徑用的暫時端點。
...
lib/words.ts（fallback）   ── 108 字靜態 seed；DATABASE_URL 沒設時填充
```

iOS 不吃 RSC，改打 `/api/words`（lite `CardWord[]`）、`/api/words/[id]`（完整）、`/api/categories`；
兩者都吃 `?lang=`（介面語言）+ `?learning=`（`zh-en` / `zh-ja` 決定目標詞 en / ja），ETag/304 + CDN 快取。

**為什麼這樣設計：**
- Admin 改完字 → `revalidateTag("words")` → 60 秒內整站（含 `/api/words` 的 unstable_cache）看到
- Client 元件不需自行 fetch；server-side 拉好放 context（web）；iOS 走快取 JSON API
...
兩個正交維度：**介面語言**（`uiLang`：zh-Hant / zh-Hans）與**學習方向**（`learningDirection`：zh-en / zh-ja，決定「要學的目標詞」是英文還是日文）。中文 UI 一樣可以學日文。

- **介面字串** — `lib/i18n.ts` 字典 + `useT()`；`<html lang>` 與 `--app-scale` 從 `user_settings` 推導
- **目標詞** — `learningDirection=zh-ja` 時取 `word_terms(language='ja')` 的 term / reading 當答案，否則用 `words.word`（英）；`lib/data.ts` `getLearningWord` / `getAllCardWords(lang, direction)` 負責挑
...
```ts
interface Word {                     // /api/words/[id]、web 詳情頁
...
interface CardWord {                 // /api/words 列表 + 卡片/圖鑑用的精簡版
  id, word,                          // word = 目標語言頭詞（依 learningDirection 給 en / ja）
  chinese, imageUrl, category, pronunciation
  reading?                           // 日文假名
  targetLanguage?: 'en' | 'ja'
  audioUrls?: Record<string,string>  // 預生成發音 clip，key = locale（en-US / en-GB / ja-JP）
}
```
...
### 兩個 deck：`image-en`（看圖選英文）/ `image-ja`（看圖選日文）

歷史上有 `recall-zh-en` / `recall-en-zh` / `cloze-1` 三 deck，2026 收斂成「看圖選答案」：圖片本身就是題目，選 4 選 1。隨**雙語學習**上線拆成兩 deck，由 `user_settings.learningDirection`（`studyDeckFor()`）決定走哪個：

```
deck_key="image-en"  card_type="回想卡"     deck_key="image-ja"  card_type="回想卡"
  front=""    // 題面是圖             front=""    // 題面是圖
  back=word（英）  // 正解              back=word_terms.term（日）  // 正解
  explanation="word [pron] — chinese"  explanation="term reading"
```

`image-en` 由 `migrate.ts:cardsForWord()` 每字一張；`image-ja` 由 `word_terms(language='ja')` 產（`INSERT … SELECT … FROM word_terms`）。`ON CONFLICT (word_id, deck_key)` 達成冪等；增字 → 自動補卡；停用字 → CASCADE 清。SRS 排程 / 熟練度按 `target_language`（en / ja）各自獨立。
...
- 熱路徑跳過第二次 `supabase.auth.getUser()`（middleware 蓋 `x-user-id`，`getCurrentUserId()` header-first 直接讀）

---

## 10b. 單詞熟練度（Mastery）

獨立於卡片 SRS 的第二層追蹤。**卡片 SRS** 決定「什麼時候再看到這張卡」；**單詞熟練度** 衡量「整體上你對這個字有多熟」，跨同字所有卡型共用。熟練度按 `target_language`（en / ja）分區 — 同一個字學英、學日各記一份。
...
- `lib/users-db.ts` — `getMasteryRow`, `upsertMastery`, `getAllMastery`, `getAllMasteryWithSchedule`（map + nextReviewAt；皆吃 `targetLanguage`）
- `lib/cards-db.ts` — `attachMasteryAndSort(userId, queue)` 弱字優先
- 頁面 / API：`/study` 顯示 delta；`/me` Top 5 + Needs Work；`/word/[id]` 登入用戶顯示熟練度條；`/api/users/mastery`（iOS 圖鑑徽章）；`/api/users/top-words`
...
/api/events (node): validate type, hash IP, INSERT
...
translate:audit    tsx scripts/translate.ts --audit-only   # 只稽核缺漏，不寫
translate:google   tsx scripts/backfill-ja-google.ts       # 日文釋義回填
translate:readings tsx scripts/backfill-ja-readings.ts     # 日文 reading 回填
```

> 預生成發音（`scripts/generate-audio.ts`）與圖檔上傳（`upload-images.ts` / `upload-local-images.ts` /
> `normalize-word-images.ts`）沒掛 npm script，直接用 `node --env-file=.env.local --import tsx scripts/<x>.ts` 跑。
...
tsx scripts/migrate.ts        # DDL idempotent + seed + cards generator
                              ...
                              # VERCEL_ENV ≠ production → 自動 skip（只在 prod build 跑）
  ↓
...
（本地 `npm run migrate` 因 `VERCEL_ENV` 未設而照常執行 — guard 只擋 Vercel 的 preview build。）
...
# 發音生成（scripts/generate-audio.ts only — runtime 不打）
GOOGLE_TTS_API_KEY            # Google Cloud 啟用 Text-to-Speech 的 API key
CHIRP_VOICE                   # 選填；Chirp 3 HD 角色名，預設 "Aoede"
```

> Google OAuth 的 client_id / secret 設在 **Supabase Dashboard**，不是專案環境變數。

### `next.config.js` 圖片白名單 + 快取

- **圖片**：`remotePatterns` 僅 Supabase Storage host（從 `NEXT_PUBLIC_SUPABASE_URL` 推導）。所有外部 host 已移除 — admin 不小心貼外部 URL 會直接被 `next/image` 拒絕。
- **快取**：公開 GET（`/api/words`、`/api/words/:id`、`/api/categories`）由 `headers()` 設 `Cache-Control: public, max-age=60`（iOS URLCache）+ `Vercel-CDN-Cache-Control: public, s-maxage=300, stale-while-revalidate=3600`（邊緣）。per-user（`/api/users/*`、`/api/study/*`、`/api/events`）一律 `private, no-store`。在 next.config.js 設而非 handler，因為 Next 對讀 searchParams 的 handler 視為 dynamic 會剝掉 handler 設的 Cache-Control。
...
backfill scripts：
  ...
  normalize-word-images.ts 統一邊距正規化
  fetch-wiki-images.mjs    legacy
```

### 發音儲存 + Chirp 3 HD pipeline（自家託管）

預生成的發音 MP3 都在 Supabase Storage 的 `word-audio` public bucket，CDN-fronted，client 從邊緣直接 stream。Runtime 不打任何 TTS。

```
scripts/generate-audio.ts（一次性 / 補跑）
  逐字 → Google Cloud TTS text:synthesize（voice = <locale>-Chirp3-HD-<CHIRP_VOICE>）
    英文 deck → headword 的 en-US + en-GB
    日文 deck → word_terms(language='ja').term 的 ja-JP
  → 上傳 word-audio/{wordId}/{locale}.mp3（upsert, cacheControl 1y）
  → UPSERT word_media(kind='audio', locale)  ON CONFLICT (word_id,kind,locale)
  → en-US 同步鏡像回 words.audio_url（back-compat）
  idempotent：已有 (word_id, locale) 音檔的 job 跳過，除非 --refresh
```

`CardWord.audioUrls` 把當前學習方向相關的 locale 打包給 client；`PronunciationButton` 優先播這些 clip，沒有才退回 Web Speech API。需 `GOOGLE_TTS_API_KEY`（+ 選填 `CHIRP_VOICE`）。
...
- **Middleware（edge）** 不能用 `cookies()` from `next/headers`，要從 `req.cookies` 讀。（`/api/events` 已從 edge 改回 `nodejs`，連同其他 route handler 一致。）
...
- **deck_key 是 `image-en` / `image-ja`**。三 deck 舊資料（`recall-zh-en` / `recall-en-zh` / `cloze-1`）已下架。選哪個 deck 由 `learningDirection` 決定（`studyDeckFor()`）；`image-ja` 卡片源自 `word_terms(language='ja')`，沒日文目標詞就生不出卡。
- **`uiLang` 不再有 `ja`**：介面只有 zh-Hant / zh-Hans。要「學日文」是 `learningDirection='zh-ja'`（換目標詞 / deck），跟介面語言正交 — 別把兩者搞混（中文 UI 也能學日文）。
- **方向分區欄位 `target_language`**：`user_learned` / `user_words` / `study_logs` 的 PK / 查詢都帶 `target_language`（en / ja）。寫這幾張表的 user-scoped 查詢漏帶方向，會把英、日進度混在一起。
- **永不信任傳入的 `x-user-id`**：middleware `updateSupabaseSession()` 一進來先 `delete` 它，驗完 cookie / Bearer 才重新 set。新增讀 header 取身分的路徑時，務必走 `getCurrentUserId()`，別自己讀 raw header。
- **zh-Hans 不存進 DB**。`word_definitions` / `category_translations` 只填 zh-Hant / ja 等；簡體由 `lib/opencc.ts` 在 read 時即時轉。
- **熱路徑避免 `supabase.auth.getUser()` 連打**。middleware 已把 `x-user-id` 蓋好，`getCurrentUserId()` header-first 直接讀；新寫熱路 API 沿用，別再各自 `getUser()`。
