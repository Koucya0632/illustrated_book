...
const OUTPUT = path.join(ROOT, "output/drugstore-publish-prep");
const AUDIO = path.join(ROOT, "output/drugstore-audio-candidates");
const CHECK_DB = process.argv.includes("--check-db");
const SOURCE = {
  candidates: "data/drugstore-series-2026-09.json",
  canonicalImages: "lib/image-urls.json",
  spans: "data/example-spans-drugstore-2026-09.json",
  images: "output/drugstore-publish-prep/image-manifest.json",
  visualReview: "output/drugstore-publish-prep/visual-review.json",
  semanticReview: "output/drugstore-publish-prep/semantic-review.json",
  audioPlan: "output/drugstore-audio-plan/manifest.json",
  audio: "output/drugstore-audio-candidates/generated-manifest.json",
  audioVerification: "output/drugstore-audio-candidates/verification-report.json",
};
...
async function databasePreflight(entries) {
  ...
  try {
    ...
    const categoryRows = await sql`SELECT id, name, name_zh FROM categories WHERE id = 'drugstore'`;
    const categoryTranslations = await sql`SELECT category_id, language, name, description FROM category_translations WHERE category_id = 'drugstore' ORDER BY language`;
...
async function main() {
  ...
  invariant(entries.every(({ category, examples }) => category === "drugstore" && examples.length === 2 && examples[0].cefrLevel === "A2" && examples[1].cefrLevel === "B1"), "category or example contract mismatch");
  ...
  const content = {
    schemaVersion: 1, series: "drugstore", productionWriteAllowed: false,
    releaseShape: { insertWordIds: entries.map(({ id }) => id), updateWordIds: [] },
    category: {
      id: "drugstore", name: "Drugstore", nameZh: "藥妝店", nameJa: "ドラッグストア", emoji: "🧴",
      description: "日本藥妝店裡常見的藥品、衛生用品與美妝產品",
      descriptionEn: "Medicines, health essentials, and beauty products in Japanese drugstores",
      descriptionJa: "日本のドラッグストアで見かける医薬品・日用品・化粧品",
      color: "from-pink-100 to-sky-100", imageUrl: categoryImage.url,
  ...
  console.log(JSON.stringify({ file: "output/drugstore-publish-prep/publish-plan.json", planDigest: plan.planDigest, status: plan.status, summary: plan.summary, databasePreflight: db && {
...
main().catch((error) => { console.error("[prepare-drugstore-publish-plan]", error); process.exitCode = 1; });
