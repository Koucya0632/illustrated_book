// Guarded one-time publisher for the reviewed 2026-09 professions series.
//
// Default mode is a SELECT-only dry run:
//   npm run professions:publish
//
...
//   npm run professions:publish -- --apply \
...
//   npm run professions:publish -- --rollback <apply-manifest.json> \
//     --confirm-host <database-fingerprint> \
//     --confirm-storage <storage-fingerprint>
...
const DEFAULT_PLAN_FILE = path.resolve(
  ROOT,
  "output/professions-publish-prep/publish-plan.json",
);
...
function verifyPreparedPlan(plan: any, planBytes: Buffer) {
  if (
    ...
    plan.series !== "professions" ||
    plan.productionWriteAllowed !== false ||
    plan.summary?.words !== 100 ||
    plan.summary?.examples !== 200 ||
    plan.summary?.images !== 100 ||
    plan.summary?.audioClips !== 900 ||
    !Array.isArray(plan.entries) ||
    plan.entries.length !== 100
  ) {
    throw new Error("invalid or incomplete professions publish plan");
  }
  ...
  if (artifacts.size !== 1000) throw new Error(`expected 1000 artifacts, found ${artifacts.size}`);
...
async function insertDraftContent(
  ...
  for (const item of plan.entries) {
    ...
    for (const relatedId of word.relatedWords) {
      if (relatedId === word.id) throw new Error(`${word.id}: self relation refused`);
      await tx`
...
async function resolveDraftExampleIds(sql: Sql | Tx, plan: any) {
  ...
  `;
  if (rows.length !== 200) throw new Error(`expected 200 stored examples, found ${rows.length}`);
...
async function assertStoredContentMatchesPlan(
  ...
  `;
  assertRowSet("profession category", categoryRows, [expectedCategory(plan)]);
  const categoryTranslations = manifest.categoryExistedBefore
    ...
    : await tx<Json[]>`
        ...
        FOR UPDATE
      `;
  assertRowSet("profession category translation", categoryTranslations, [{
    category_id: plan.category.id,
    language: "ja",
    name: plan.category.nameJa,
    description: plan.category.descriptionJa,
...
function buildStorageObjects(plan: any, exampleIds: Record<string, string>): StorageObjectPlan[] {
  ...
  if (objects.length !== 1000 || new Set(objects.map(({ key }) => key)).size !== 1000) {
...
async function stageStorage(manifestFile: string, manifest: ApplyManifest) {
  ...
  for (const [index, object] of manifest.storageObjects.entries()) {
    ...
    if ((index + 1) % 25 === 0 || index + 1 === manifest.storageObjects.length) {
      console.log(`[professions:publish] Storage ${index + 1}/${manifest.storageObjects.length}`);
    }
  }
...
async function publishDraft(tx: Tx, plan: any, manifest: ApplyManifest) {
  ...
  `;
  if (states.length !== 100 || states.some(({ status }) => status !== "draft")) {
    throw new Error("publish refused: all 100 profession rows must still be draft");
  }
  ...
  if (published.length !== 100) {
    throw new Error(`publish refused: expected to publish 100 rows, changed ${published.length}`);
  }
  ...
  const [row] = await sql<Record<string, number>[]>`
    SELECT
      ...
      (SELECT count(*)::int FROM user_settings
        WHERE study_category = 'professions'
           OR 'professions' = ANY(regexp_split_to_array(study_categories, '\\s*,\\s*'))
      ) AS user_settings
  `;
  return row;
}
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      if (states.length !== ids.length) {
        throw new Error("rollback refused: only part of the profession series exists");
      }
      const phase = states.every(({ status }) => status === "published")
        ...
        : states.every(({ status }) => status === "draft")
          ? "draft"
          : null;
      if (!phase) throw new Error("rollback refused: profession row states are mixed");
      ...
      if (userReferenceCount > 0 && !ALLOW_USER_DATA_LOSS) {
        throw new Error(
          `rollback refused: ${userReferenceCount} user-data references exist; inspect them before considering --allow-user-data-loss`,
        );
      }
      ...
      if (outsideReuse[0].count > 0) {
        throw new Error("rollback refused: a profession sentence is now reused outside the series");
      }
      ...
      if (sharedDefinitions.length > 0) {
        throw new Error("rollback refused: a profession sentence is reused by an outside definition");
      }
      ...
      if (inboundRelations.length > 0) {
        throw new Error("rollback refused: outside words now link to the profession series");
      }
      ...
      if (outsideSpanLinks.length > 0) {
        throw new Error("rollback refused: outside sentence spans now link to the profession series");
      }
      await tx`
    ...
    writeJsonAtomic(manifestFile, manifest);
    console.log(
      `[professions:publish] rolled back ${ids.length} words; ` +
...
async function main() {
  ...
  if (
    actionManifest &&
    (actionManifest.schemaVersion !== 1 || actionManifest.series !== "professions")
  ) {
    throw new Error("invalid professions recovery manifest");
  }
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          storageObjects: plan.summary.images + plan.summary.audioClips,
    ...
    if (!APPLY) {
      console.log(
        `[professions:publish] dry run only; no Storage or database changes. ` +
    ...
    if (preflight.alreadyPublished && !RESUME_FILE) {
      throw new Error("all profession rows are already published; use the audit workflow instead");
    }
    if (preflight.resumableDraft) {
      throw new Error(
        "committed profession drafts are from an obsolete publisher flow; rollback them before continuing",
      );
    }
    ...
    try {
      ...
      if (preflight.alreadyPublished) {
        ...
        writeJsonAtomic(manifestFile, manifest);
        console.log(
          `[professions:publish] recovered completed publication; manifest: ${relative(manifestFile)}`,
        );
        return;
      }
      ...
      writeJsonAtomic(manifestFile, manifest);
      console.log(`[professions:publish] published 100 words; manifest: ${relative(manifestFile)}`);
    } catch (error) {
...
main().catch((error) => {
  console.error("[professions:publish] failed:", error);
  process.exitCode = 1;
});
