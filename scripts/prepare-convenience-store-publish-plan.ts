// Build an immutable, local-first publication plan for the convenience-store
...
// current convenienceStore snapshot that a later compare-and-swap apply must preserve.
...
const EXISTING_ID = "convenience-store";
const NEW_COUNT = 33;
const OUTPUT_DIR = path.resolve(ROOT, "output/convenience-store-publish-prep");
const PLAN_FILE = path.join(OUTPUT_DIR, "publish-plan.json");
const SPANS_FILE = path.join(ROOT, "data/example-spans-convenience-store-2026-09.json");
...
const AUDIO_ROOT = path.join(ROOT, "output/convenience-store-audio-candidates");
...
async function databasePreflight(entries: Entry[]) {
  ...
  try {
    ...
    const category = await sql<Record<string, unknown>[]>`
      ...
      FROM categories WHERE id = 'convenience-store'
    `;
    ...
    `;
    const convenienceStoreWord = await sql<Record<string, unknown>[]>`
    ...
    `;
    const convenienceStoreTerms = await sql<Record<string, unknown>[]>`
    ...
    `;
    const convenienceStoreDefinitions = await sql<Record<string, unknown>[]>`
    ...
    `;
    const convenienceStoreExamples = await sql<Record<string, unknown>[]>`
      SELECT id::text, word_id, sentence, cefr_level, sort_order
      FROM word_examples WHERE word_id = ${EXISTING_ID} ORDER BY sort_order, id
    `;
    const convenienceStoreExampleIds = convenienceStoreExamples.map(({ id }) => String(id));
    const convenienceStoreEnglishSentences = convenienceStoreExamples.map(({ sentence }) => String(sentence));
    const convenienceStoreTranslations = convenienceStoreExampleIds.length === 0 ? [] : await sql<Record<string, unknown>[]>`
      SELECT example_id::text, language, translation FROM word_example_translations
      WHERE example_id::text = ANY(${convenienceStoreExampleIds}) ORDER BY example_id, language
    `;
    const convenienceStoreJapaneseSentences = convenienceStoreTranslations
      .filter(({ language }) => language === "ja")
      .map(({ translation }) => String(translation));
    const convenienceStoreSpans = await sql<Record<string, unknown>[]>`
      SELECT sentence_language, sentence, sort_order, text, base_form,
             part_of_speech, reading, word_id, version
      FROM sentence_spans
      WHERE (sentence_language = 'en' AND sentence = ANY(${convenienceStoreEnglishSentences}))
         OR (sentence_language = 'ja' AND sentence = ANY(${convenienceStoreJapaneseSentences}))
      ORDER BY sentence_language, sentence, sort_order
    `;
    const convenienceStoreGlosses = await sql<Record<string, unknown>[]>`
    ...
    `;
    const convenienceStoreRelations = await sql<Record<string, unknown>[]>`
    ...
    `;
    const convenienceStoreCategories = await sql<Record<string, unknown>[]>`
      SELECT word_id, category_id, is_primary
      FROM word_categories WHERE word_id = ${EXISTING_ID} ORDER BY category_id
    `;
    const convenienceStoreMedia = await sql<Record<string, unknown>[]>`
    ...
    `;
    const convenienceStoreExampleMedia = convenienceStoreExampleIds.length === 0 ? [] : await sql<Record<string, unknown>[]>`
      SELECT example_id::text, locale, url, storage_path, mime_type, duration_ms, model
      FROM word_example_media WHERE example_id::text = ANY(${convenienceStoreExampleIds})
      ORDER BY example_id, locale
    `;
    const convenienceStoreCards = await sql<Record<string, unknown>[]>`
      SELECT id::text, word_id, card_type, front, back, explanation, tags, deck_key
      FROM cards WHERE word_id = ${EXISTING_ID} ORDER BY id
    `;
    const [convenienceStoreUserReferences] = await sql<Record<string, number>[]>`
      SELECT
    ...
    const convenienceStoreCandidate = candidateRows.find(({ id }) => id === EXISTING_ID);
    const convenienceStoreBeforeCatalog = {
      words: convenienceStoreWord,
      terms: convenienceStoreTerms,
      definitions: convenienceStoreDefinitions,
      examples: convenienceStoreExamples,
      translations: convenienceStoreTranslations,
      spans: convenienceStoreSpans,
      glosses: convenienceStoreGlosses,
      relations: convenienceStoreRelations,
      categories: convenienceStoreCategories,
      media: convenienceStoreMedia,
      exampleMedia: convenienceStoreExampleMedia,
      cards: convenienceStoreCards,
    };
    const convenienceStoreBefore = {
      ...convenienceStoreBeforeCatalog,
      userReferences: convenienceStoreUserReferences,
    };
    const existingSpanKeys = new Set(
      convenienceStoreSpans.map(
        ({ sentence_language, sentence }) => `${sentence_language}\u0000${sentence}`,
      ),
    );
    const unexpectedSpanCollisions = spanCollisions.filter(
      ({ sentence_language, sentence }) =>
        !existingSpanKeys.has(`${sentence_language}\u0000${sentence}`),
    );
    return {
      ...
      headwordConflicts,
      spanCollisions,
      unexpectedSpanCollisions,
      convenienceStoreBefore,
      convenienceStoreBeforeDigest: digest(convenienceStoreBeforeCatalog),
      readyForGuardedApply:
        Object.values(tables).every(Boolean) &&
        category.length === 0 &&
        newConflicts.length === 0 &&
        headwordConflicts.length === 0 &&
        convenienceStoreCandidate?.status === "published" &&
        convenienceStoreCandidate?.category === "street" &&
        convenienceStoreWord.length === 1 &&
        convenienceStoreExamples.length === 2 &&
        unexpectedSpanCollisions.length === 0,
    };
  } finally {
    await sql.end();
  }
...
async function main() {
  ...
  if (entries.length !== 34 || new Set(entries.map(({ id }) => id)).size !== 34) {
    throw new Error("expected exactly 34 unique convenience-store entries");
  }
  if (entries.filter(({ id }) => id !== EXISTING_ID).length !== NEW_COUNT) {
    throw new Error(`expected exactly ${NEW_COUNT} new entries plus convenience-store`);
  }
  if (
    visualReview.summary.passedWordImages !== 34 ||
    visualReview.summary.failedWordImages !== 0 ||
    visualReview.summary.passedCategoryImages !== 1
  ) {
  ...
  const plan = {
    ...
    category: {
      ...
      descriptionJa: "日本のコンビニで見かける設備・サービス・商品",
    ...
    applyOrder: [
      "repeat the SELECT-only database preflight and compare the full convenienceStore snapshot digest",
      "stage 340 content-addressed image/audio objects and verify their remote bytes",
      "insert 33 draft words and update convenienceStore in one serializable transaction without changing its word ID or card IDs",
      "replace only reviewed catalogue-owned convenienceStore children using compare-and-swap checks",
    ...
    rollbackContract: {
      database: "Restore convenienceStore from databasePreflight.convenienceStoreBefore and delete only the 33 inserted IDs, all under compare-and-swap guards against the applied snapshot.",
      cards: "Preserve existing convenienceStore card IDs so user_cards references remain valid; restore only their reviewed mutable fields.",
      ...
      stopConditions: [
        "the convenienceStore snapshot digest differs",
        "any of the 33 new IDs already exists",
        "the category already exists with unreviewed content",
        "an unexpected sentence-span collision exists outside the preserved convenience-store examples",
  ...
  console.log(JSON.stringify({
    ...
    databasePreflight: databasePreflightResult
      ? {
          ...
          preservedSpanRows: databasePreflightResult.spanCollisions.length,
          unexpectedSpanCollisions: databasePreflightResult.unexpectedSpanCollisions.length,
          categoryExists: Boolean(databasePreflightResult.category),
          convenienceStoreBeforeDigest: databasePreflightResult.convenienceStoreBeforeDigest,
        }
      : "not requested",
  }, null, 2));
}
...
main().catch((error) => {
  console.error("[prepare-convenience-store-publish-plan] failed:", error);
  process.exitCode = 1;
});
